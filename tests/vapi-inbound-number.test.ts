...
describe("asking Vapi for an inbound number", () => {
  it("asks for a Vapi-provided number bound to the agent's assistant", async () => {
    ...
    expect(bodyAt(0)).toEqual({ provider: "vapi", assistantId: "asst_1", name: "Frisørstuen" });
    expect(number).toEqual({ id: "num_1", number: "+15139407163" });
  });

  // Vapi picks one itself when no area code is given — sending an empty one
  // would be a request for an area code called "".
  it("leaves the area code out entirely when none was chosen", async () => {
    await createVapiManagedNumber({ assistantId: "asst_1" });

    expect(bodyAt(0)).toEqual({ provider: "vapi", assistantId: "asst_1" });
    expect(Object.keys(bodyAt(0))).not.toContain("numberDesiredAreaCode");
  });

  it("passes a chosen area code through under Vapi's own field name", async () => {
    await createVapiManagedNumber({ assistantId: "asst_1", areaCode: "415" });

    expect(bodyAt(0).numberDesiredAreaCode).toBe("415");
  });
