...
import { createVapiManagedNumber, ensureInboundAssistant } from "@/lib/vapi";
...
export const POST = withErrorHandling(async (request) => {
  ...
  // Vapi's own errors travel out as-is (see lib/vapi/client.ts): running out
  // of free numbers is a platform-level fact an admin has to act on, and
  // rewording it would only hide which limit was hit.
  const number = await createVapiManagedNumber({
    assistantId,
