...
import { buildGoogleAuthUrl, buildOAuthState, cookieNameForProvider } from "@/lib/calendar";
import { ApiError } from "@/types/errors";

export const dynamic = "force-dynamic";

// Kicks off the Google Calendar OAuth flow for one widget: verify the
// widget belongs to the caller, stash a CSRF nonce in a short-lived cookie
// (checked again in the callback route), and redirect to Google's consent
// screen.
export const GET = withErrorHandling(async (request) => {
  ...
  cookies().set(cookieNameForProvider("google"), nonce, {
    httpOnly: true,
